...
const ResultsView = ({
  ...
  const renderActive = () => {
    if (activeTab === 'listening') return <ListeningAnalysis feedbacks={aiFeedback.listening} />;
    if (activeTab === 'reading') return <ReadingAnalysis feedbacks={aiFeedback.reading} />;
    if (activeTab === 'writing') return <WritingAnalysis feedbacks={aiFeedback.writing} />;
    if (activeTab === 'speaking') return <SpeakingAnalysis feedbacks={aiFeedback.speaking} />;
    return null;
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8">
      <div className="text-center mb-8">
        ...
        {aiFeedback.listening.length + aiFeedback.reading.length + aiFeedback.speaking.length + aiFeedback.writing.length > 0 ? (
          <p className="text-gray-600">Powered by AI analysis for each section</p>
        ) : (
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {['listening', 'reading', 'writing', 'speaking'].map((tab) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg border ${activeTab === tab ? 'bg-teal-600 text-white border-teal-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
            >
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10 ">
        <Link to="/dashboard" className="bg-teal-600 text-white px-8 py-3 rounded-lg hover:bg-teal-700 font-semibold text-center">Back to Dashboard</Link>
        <button onClick={onRetake} className="bg-gray-600 text-white px-8 py-3 rounded-lg hover:bg-gray-700 font-semibold">Take Another Test</button>
      </div>
    </div>
  );
};
