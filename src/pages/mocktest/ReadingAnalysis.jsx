...
const ReadingAnalysis = ({ feedbacks }) => {
  return (
    <div className="space-y-6">
      {!Array.isArray(feedbacks) || feedbacks.length === 0 ? (
        <p className="text-gray-600">No AI analysis available for Reading.</p>
      ) : (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <h5 className="text-lg font-semibold">
                Reading Passage {idx + 1}
              </h5>
              <span className="text-teal-600 font-bold">
                Band: {+(fb?.overall_score || 0).toFixed(1)}
              </span>
            </div>
            {fb?.overall_feedback && (
                  ...
                  {fb.question_analysis.map((qa, qIdx) => (
                    <li key={qIdx} className="bg-gray-50 p-3 rounded">
                      ...
                      <div className="text-sm text-gray-700">
                        Your answer:{' '}
                        {String(qa?.student_answer || 'No answer provided')}
                      </div>
                      <div className="text-sm text-gray-700">
                        Correct: {String(qa?.correct_answer || '')}
                      </div>
                      <div
                        className={`text-sm ${
